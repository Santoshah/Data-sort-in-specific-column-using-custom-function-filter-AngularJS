...
app.controller('searchData', ['$scope', function($scope){
	var employees = [
	...
	$scope.searchItem = function(item){
		if($scope.searchFilter == undefined) {
			...
			if(item.name.toLowerCase().indexOf($scope.searchFilter.toLowerCase()) != -1 ||
				item.salary.toString().toLowerCase().indexOf($scope.searchFilter.toLowerCase())  != -1) {
				return true;
			}
		}
	}
